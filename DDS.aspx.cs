...
public partial class _default : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                ActaF.HRef = tramite.iACTA.ToString(); if (ActaF.HRef != "") { ActaF.InnerText = lbl32.InnerText.ToString(); } else { ActaF.Visible = false; }
                ...
                chkDictamenTecnico.Checked = tramite.DICTECCUMPLE;
                ...
                if (riesgo.Text == "1")
                {
                    ...
                    if (statos.Text == "10" || statos.Text == "10")
                    {
                        if (statos.Text == "10")
                        {
    ...
    protected void btnGuardarDatos_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (tramite.GuardarSinStaus_tram())
            { Response.Redirect("ldds.aspx"); }
        }
        catch (Exception Ex)
        {
            LblMsg.Text = Ex.Message;
    ...
    protected void btnAprobado_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (FileDicTec.HasFile)
            {
                string Hoy = DateTime.Now.ToString("dd-MM-yyyy H-mm-ss");
                string fileName = "DicTecnico" + folio.Text + System.IO.Path.GetExtension(FileDicTec.FileName);
                idictec = "uploads/" + fileName + "";
                FileDicTec.SaveAs(Server.MapPath("~/") + idictec);
            }
    ...
    protected void btnPrevision_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (FileDicTec.PostedFile != null && FileDicTec.HasFile)
            {
                SqlConnection cnna = new SqlConnection();
                cnna.ConnectionString = Principal.CnnStr0;
                cnna.Open();
                //SqlCommand cmda = new SqlCommand(" Insert Into bitaseg.imgchar (folio) values ('" + folio.Text + "')");
                SqlCommand cmda = new SqlCommand("if  not exists(select folio from bitaseg.imgchar where folio = ('" + folio.Text + "')) begin Insert Into bitaseg.imgchar(folio) values ('" + folio.Text + "') end");



                cmda.Connection = cnna;
            ...
            if (FileDicTec.HasFile)
            {
                ...
                {
                    string Hoy = DateTime.Now.ToString("dd-MM-yyyy H-mm-ss");
                    string fileName = "Dictec" + folio.Text + System.IO.Path.GetExtension(FileDicTec.FileName);
                    FileDicTec.SaveAs(Server.MapPath("~/uploads/") + fileName);


                    SqlConnection cnna = new SqlConnection();
                    cnna.ConnectionString = Principal.CnnStr0;
                    cnna.Open();
                    SqlCommand cmda = new SqlCommand("Update bitaseg.imgchar SET dictec = ('uploads/" + fileName + "') where folio = " + folio.Text + " ");
    ...
    protected void btnImprimir_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Tramites tramite = new Tramites(Convert.ToInt32(Request.Params["id"]))
            {
                Statos = Convert.ToInt32(statos.Text),
                Curp = txtCurp.Text.ToUpper(),
                Nombre = txtNombre.Text.ToUpper(),
                Telfij = txtTelfij.Text.ToUpper(),
                Telmov = txtTelmov.Text.ToUpper(),
