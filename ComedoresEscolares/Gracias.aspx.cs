...
public partial class Gracias : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            try
            {
                ...
                if (comedor.Descargado == false)
                {
                    SqlConnection cna = new SqlConnection();
                    cna.ConnectionString = Principal.CnnStr0;
                    cna.Open();
                    SqlCommand cma = new SqlCommand("UPDATE bitaseg.Comedores_Escuelas SET descargado = 1  where id_comedor = ('" + id_decrypt + "') ");
                    cma.Connection = cna;
                    cma.ExecuteNonQuery();
                    cna.Close();
    ...
    void GenerarPDF(Page page)
    {
        try
        {
            var encrypt = Request.Params["id"].ToString();
            ...
            var id_decrypt = cripto.Decrypt(Request.Params["id"].ToString());
            Comedores comedor = new Comedores(Convert.ToInt32(id_decrypt));
            Escuelas escuela = new Escuelas(comedor.ClaveCT);
            ...
            string nombrecompleto = comedor.Nombre+" "+comedor.Apellidop+" "+comedor.Apellidom;
            string imageFilePathHead = page.Server.MapPath("../img/test2.png");
            string imgFilePath = page.Server.MapPath("../img/RegistroNacionalDePoblacion.png");



           
            Image imgHead = new Image(ImageDataFactory.Create(imageFilePathHead));
            Image img = new Image(ImageDataFactory.Create(imgFilePath));
            ...
            Cell numero_folio = new Cell(1, 445).Add(new Paragraph(comedor.Folio.ToString()).SetTextAlignment(TextAlignment.LEFT).SetFontSize(12).SetBold());
            ...
            Paragraph comprobante1;
             comprobante1 = new Paragraph("Comprobante de registro                                                CAPACITACIÓN PARA UNIDADES DE CONSUMO ESCOLAR").SetTextAlignment(TextAlignment.CENTER).SetFontSize(20); 
            ...
            lblnombre = new Cell(1, 366).Add(new Paragraph("Datos del responsable:").SetFontSize(12).SetFont(HelveticaNeueLTstd_Bd)); 
            table.AddCell(lblnombre);


            Cell lblestablecimiento;
            lblestablecimiento = new Cell(1, 566).Add(new Paragraph("Datos de la institución educativa:").SetFontSize(12).SetFont(HelveticaNeueLTstd_Bd)); 
            ...
            Cell nombreestablecimiento = new Cell(2, 471).Add(new Paragraph(escuela.Nombre));
            ...
            //TERCER FILA
            Cell tel = new Cell(3, 120).Add(new Paragraph(comedor.Tel));
            table.AddCell(tel);
            Cell correo = new Cell(3, 246).Add(new Paragraph(comedor.Correo));
            table.AddCell(correo);
            Cell municipio = new Cell(3, 566).Add(new Paragraph("Municipio: " + escuela.Municipio));
            ...
            Cell localidad = new Cell(4, 566).Add(new Paragraph("Localidad: " + escuela.Localidad));
            ...
            Cell colonia = new Cell(5, 283).Add(new Paragraph("Turno: " + escuela.Turno));
            table.AddCell(colonia);
            Cell cp = new Cell(5, 283).Add(new Paragraph("Tipo: "+escuela.Tipo));
            table.AddCell(cp);

            Cell relleno5 = new Cell(5, 68).Add(new Paragraph("").SetFontSize(12).SetFont(HelveticaNeueLTstd_Bd));
            table.AddCell(relleno5);
            //FILA PARA Comedores
            Cell vacio400_row4_comedores = new Cell(6, 366).Add(new Paragraph(""));
            table.AddCell(vacio400_row4);
            Cell ClaveCTT_Texto= new Cell(6, 250).Add(new Paragraph("Clave del centro de trabajo: "));
            table.AddCell(ClaveCTT_Texto);
            Cell ClaveCTT_Valor = new Cell(6, 316).Add(new Paragraph(escuela.ClaveCT));
            table.AddCell(ClaveCTT_Valor);


            //SEXTA FILA
            //Cell vacio400_row6 = new Cell(6, 366).Add(new Paragraph(""));
            //table.AddCell(vacio400_row6);
            //Cell calle = new Cell(6, 338).Add(new Paragraph("Calle: " + "Calle de prueba"));
            //table.AddCell(calle);
            //Cell numext = new Cell(6, 114).Add(new Paragraph("Num.Ext.: " + "N.E. P"));
            //table.AddCell(numext);
            //Cell numint = new Cell(6, 114).Add(new Paragraph("Num.Int.:     " + "N.I P"));
            ...
            //Cell entrecalles = new Cell(7, 471).Add(new Paragraph("Entrecalles Prueba"));
            ...
            vacio400_row4_comedores.SetBorder(Border.NO_BORDER);
            ...
            //Cell tg = new Cell(1, 431).Add(new Paragraph("TRÁMITE GRATUITO")).SetTextAlignment(TextAlignment.RIGHT).SetFontSize(12);
            //tabla_cabecera.AddCell(tg);
            BarcodeQRCode qrCode = new BarcodeQRCode("https://coepristam.saludtamaulipas.net/sistema/Verificacion.aspx?id="+ encrypt);
            ...
            Paragraph QR = new Paragraph().Add(barcodeImage);
            //QR.SetFixedPosition(50, 400);
            imgHead.SetMarginLeft(20).SetMarginRight(20).SetMarginTop(20);
            tabla_cabecera.SetMarginLeft(20).SetFontColor(ColorConstants.RED);
            ...
            document.Add(QR.SetFixedPosition(230,50,1000));
