    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                RFC_prop.InnerHtml = tramite.RFC_Prop;
                Curp.InnerHtml = tramite.Curp;
                id_tramite.Text = tramite.IdTramite.ToString();
                NombreCompleto.InnerText = tramite.Nombre +' ' + tramite.Apellidop+' ' + tramite.Apellidom;
                TelefonoFij.InnerText = tramite.Telfij;
                TelefonoMov.InnerText = tramite.Telmov;
                Correo.InnerText = tramite.Correo;
                Rfc.InnerText = tramite.Rfc;
                Nombreest.InnerText = tramite.Nombreest;
                giro.InnerText = tramite.Razonsocial;
                Municipio.InnerText = tramite.Municipio;
                Localidad.InnerText = tramite.Localidad;
                Colonia.InnerText = tramite.Colonia;
                CP.InnerText = tramite.CP;
                Calle.InnerText = tramite.Calle+" "+ tramite.Numero;
                referencias.InnerText = tramite.Referencias;
                Telest.InnerText = tramite.Telest;
                Correoest.InnerText = tramite.Correoest;
                RFC_resp.InnerText = tramite.Rfc_R;
                Nombre_resp.InnerText = tramite.Rnombre + " " + tramite.Rapellidop + " " + tramite.Rapellidom;
                grado.InnerText = tramite.Rgrado;
                cedula.InnerText = tramite.Rcedula;
