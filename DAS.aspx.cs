...
public partial class _default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                id_responsable.Text = tramite.Idresponsable.ToString();
                ...
                Prevencion.HRef = tramite.iPREVENCION.ToString(); if (Prevencion.HRef != "") { } else { Prevencion.Visible = false; }
                DictamenDocumental.HRef = tramite.iDIDOC.ToString(); if (DictamenDocumental.HRef != "") { } else { DictamenDocumental.Visible = false; }
                ...
                chkPrevencion.Checked = tramite.PREVENCION; if (chkPrevencion.Checked == false) { prv.Visible = false; }
                ...
                var chkVerificacion = tramite.RequiereVerificacion;
                bool prevencion = tramite.PREVENCION;
                switch (statos.Text)
                {
                    case "15":
                        ...
                        btn.Visible = false;
                        dictamenjj.Visible = true;
                        if (chkVerificacion == false) { sdivacta.Visible = false; sdict.Visible = false; grid2.Visible = false; }
                        break;
                    case "16":
                        btnAprobado.Visible = false;
                        btnRechazado.Visible = false;
                        btnPrevision.Text = "Marcar Como Impreso";
                        btn.Visible = true;
                        dictamenjj.Visible = true;
                        if (chkVerificacion == false) { sdivacta.Visible = false; sdict.Visible = false; grid2.Visible = false; }
                    ...
                    case "19":
                        btnAprobado.Visible = true;
                        btnAprobado.Text = "Recibir desde UEAS";
                        btnAprobado.CssClass = "myButton";
                        btnRechazado.Visible = false;
                    ...
                    case "1004":
                        ...
                        btnPrevision.CausesValidation = false;
                        sdict.Visible = false;
                        grid2.Visible = false;
                        sdivacta.Visible = false;
                        foliolic.Visible = false;
                        if (chkPrevencion.Visible == true) { btnAprobado.Visible = false; btnRechazado.Visible = false; }
                        else { if (prevencion == true) { btnAprobado.Visible = false; btnRechazado.Visible = false; } }
                        break;

                    case "1017":    
                        btnRechazado.Visible = false;
                        //btnPrevision.CssClass = "myButtonAprobar";
                        btnPrevision.Visible = false;
                        btn.Visible = true;
                        dictamenjj.Visible = false;
                    ...
                    default:
                        ...
                        sdivacta.Visible = false;
                        foliolic.Visible = false;
                        desaparecer.Visible = false;
                        DictamenDocumental.Visible = false;
                        dictamenjj.Visible = false;
                        break;
    ...
    protected void btnAprobado_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (riesgo.Text == "1")
            {
                if (statos.Text == "15" || statos.Text == "19")
                {
                    if (statos.Text == "15")
                    {
                        a = 22;
            ...
            SqlCommand cma = new SqlCommand("UPDATE bitaseg.tramites SET fecha_expedicion = (@fecha), clave_dals = ('"+txtClave_dals.Text+"'), foliolicencia = ('"+txtFoliolicencia.Text+"') where folio = ('" + folio.Text + "')  UPDATE bitaseg.ResponsablesSanitarios  SET rgrado= ('"+txtRgrado.Text+"') where id_responsable = ('" + id_responsable.Text + "') ");
            cma.Parameters.Add("@fecha", SqlDbType.DateTime).Value = txtFecha_Expedicion.Text.ToString();
            cma.Connection = cna;
            cma.ExecuteNonQuery();
            cna.Close();
            }
    ...
    protected void btnRechazado_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cna.Open();
            //SqlCommand cmda = new SqlCommand(" Insert Into bitaseg.imgchar (folio) values ('" + folio.Text + "')");
            SqlCommand cma = new SqlCommand("UPDATE bitaseg.tramites SET rechazo = (('" + rechazo.Text + "'))  where folio = ('" + folio.Text + "') ");
    ...
    {
        if (prv.Visible == false) { prv.Visible = true; Response.Write("<script>alert('Favor de digitalizar los oficios de prevención.')</script>");
            SqlConnection cna = new SqlConnection();
            cna.ConnectionString = Principal.CnnStr0;
            cna.Open();
            SqlCommand cma = new SqlCommand("UPDATE bitaseg.tramites SET prevencion = 1 where folio = ('" + folio.Text + "') ");
            cma.Connection = cna;
            cma.ExecuteNonQuery();
            cna.Close();
        }
        ...
        try
        {
            ...
            if (statos.Text == "1017")
            {
                ...
                try
                {
                    ...
                    cna.Open();
                    //SqlCommand cmda = new SqlCommand(" Insert Into bitaseg.imgchar (folio) values ('" + folio.Text + "')");
                    SqlCommand cma = new SqlCommand("UPDATE bitaseg.tramites SET foliolicencia = (('" + txtFoliolicencia.Text + "'))  where folio = ('" + folio.Text + "') ");
    ...
    //    DateTime date1;
    //    if (DateTime.TryParse(this.txtFecha_Expedicion.Text, out date1))
    //    {
    //        Response.Write("<script>alert('" + date1 + "')</script>");
    //    }
    //    else
    //    {
