...
public partial class _default : System.Web.UI.Page
{
            ...
            if (!Page.IsPostBack)
            {
                ...
                else
                {
                    if (riesgo.Text == "2")
                    {
                        ...
                        if (statos.Text == "1020" || statos.Text == "1021")
                        {
                            if (statos.Text == "1020")
                            {
                                btnAprobado25.Visible = true;
    ...
    protected void btnAprobado_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            { Response.Redirect("lcorrespondencia.aspx"); }
        }
        catch (Exception Ex)
        {
            LblMsg.Text = Ex.Message;
    ...
    private void CargarCombo()
    {
            ...
            cnn.Open();
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = "SELECT * FROM bitaseg.coordinaciones";
            cmd.Connection = cnn;
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            Dropdownlist1.DataTextField = "nombre_coordinacion";
            Dropdownlist1.DataValueField = "id_coordinacion";
